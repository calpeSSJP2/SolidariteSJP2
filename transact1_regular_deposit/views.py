...
from .services import LegacyAccountUpdateService
...
class LegacyAccountSearchView(LoginRequiredMixin, TemplateView):
    template_name = "transact1_regular_deposit/account_search.html"
...
class LegacyAccountImportView(
    LoginRequiredMixin,
    UserPassesTestMixin,
    TemplateView
):
    template_name = "transact1_regular_deposit/legacy_form.html"

    def test_func(self):
        return self.request.user.has_any_role("itadmin", "officer")

    def dispatch(self, request, *args, **kwargs):
        self.account = get_object_or_404(
            MemberAccount.objects.select_related(
                "member",
                "member__user"
            ),
    ...
    def get(self, request, *args, **kwargs):
        form = LegacyAccountUpdateForm(initial={
    ...
    def post(self, request, *args, **kwargs):
        form = LegacyAccountUpdateForm(request.POST)

        if form.is_valid():
            data = form.cleaned_data
            data["account_obj"] = self.account

            LegacyAccountUpdateService.update_account(data)

            return redirect("transact1_regular_deposit:legacy-search")
