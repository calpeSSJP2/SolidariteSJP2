...
class LoanPaymentView2(FormView): ##This return only one loans
    model = LoanPayment
...
class LoanPaymentView(FormView):
    ...
    def get_member_account(self):
        member_id = (
            self.kwargs.get('member_account')
            or self.request.POST.get('member_account')
        )
        return get_object_or_404(MemberAccount, pk=member_id)

    def get_member_loans(self, member_account):
        """
        Returns all unpaid loans for borrower
        (regular + emergency + top-ups).
        """
        return Loan.objects.unpaid_loans_for_account(member_account)

    def get_peer_loans(self, member_account, for_update=False):
        qs = PeerToPeerLoan.objects.filter(
            borrower=member_account,
            is_fully_paid=False
        ).select_related("lender")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not member_loans:
            messages.error(
                request,
                "No active or approved loans found for this member."
            )
            return redirect(request.path)
        ...
        for form in peer_forms:
            amount = form.data.get(form.add_prefix('amount'))
            if amount and not form.is_valid():
                peer_valid = False

        if not (loan_valid and peer_valid):
            context = self.get_context_data(form=loan_form)
            context['peer_forms'] = list(zip(peer_loans, peer_forms))
            return self.render_to_response(context)
        ...
        if peer_total > loan_amount:
            loan_form.add_error(
                'amount',
                "Total peer repayments cannot exceed loan payment amount."
            )
        ...
        with transaction.atomic():
            ...
            # LOCK ALL LOANS (critical for safety)
            locked_loans = list(
            ...
            for loan in member_loans:
                ...
                elif loan.total_paid > 0:
                    loan.status = Loan.LoanStatus.ACTIVE

                loan.save(update_fields=['status'])

        messages.success( request,  "Payments successfully recorded."   )
